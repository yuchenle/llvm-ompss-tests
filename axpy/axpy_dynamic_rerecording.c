...
int main(int argc, char** argv)
{
    ...
    #pragma omp single
    {
      ...
      #ifdef TDG
      #pragma omp taskgraph tdg_type(dynamic) if(i==5)// num_preallocs(2)
      #endif
      {
        if(i>2)
        {
    ...
    if(tasks_added !=5){
         printf("\033[31;1mAXPY dynamic recording test: failed %d \n",tasks_added);
         fclose(fp);
         exit(1);
    }
    for (int i = 0; i < N; ++i){
        ...
        {
            printf("\033[31;1mAXPY dynamic recording test: failed %f \n", y[i]);
            //fprintf(fp, "%d::Error!\n", omp_get_max_threads());
            fclose(fp);
            exit(1);
        } 
    }
    printf("\033[32;1mAXPY dynamic recording test: correct \n");
    fclose(fp);
